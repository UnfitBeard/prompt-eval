...
async def create_template(
    ...
    try:
        ...
        return APIResponse(
            success=True,
            data={"template_id": str(result.inserted_id), "message": "Template created successfully"},
        )
...
async def list_templates(
    ...
    try:
        ...
        cursor = mongodb.db.templates.find(query).sort("createdAt", -1).skip(skip).limit(limit)
...
async def update_template(
    ...
    try:
        ...
        update_dict = {k: v for k, v in template.dict(exclude_unset=True).items() if v is not None}
